...
const PricingCard = ({
  ...
  const handlePopup = async () => {
    ...
    try {
      // In local development the Clerk Checkout UI often cannot render
      // when billing is not configured in the Clerk dashboard. Avoid
      // calling the internal checkout in development to prevent the
      // dev-only notice and a noisy console.
      if (process.env.NODE_ENV === "development") {
        try {
          // For local development, simulate an upgrade so devs can test Pro features.
          window.localStorage.setItem("DEV_FORCE_PRO", "1");
          window.location.reload();
          return;
        } catch (e) {
          console.info("Skipping Clerk checkout in development (billing may be disabled).", e);
          return;
        }
      }
